## [2026-03-01] 罗盘初始位置自动避让煞气点

### 变更内容

**问题**：部分关卡进入后，煞气点位置恰好与罗盘初始位置（屏幕中心）重叠，导致解煞选项弹窗立即弹出，影响游戏体验。

**解决方案**：加载关卡时自动检测并调整位置，避免罗盘与煞气点重叠。

**实现逻辑**：

1. **Mobile 端**（罗盘固定在屏幕中心）：
   - 检测屏幕中心位置是否在任何煞气点的检测范围内
   - 如果有碰撞风险，自动偏移房间容器，把煞气点"推"开
   - 玩家可以自由拖动图片探索

2. **Web 端**（罗盘位置相对于图片）：
   - 计算一个安全的初始罗盘位置
   - 通过回调通知父组件更新 `compassPosition`
   - 优先从预定义的候选位置中选择（四角、四边中点等）
...
**安全距离计算**：
```
safeDistance = sha.radius + compassRadius + 0.05 (缓冲区)
```

### 影响范围
- `src/frontend/feng-shui-8-bit/src/client/game/GameStage.tsx` - 新增 `calculateSafeInitialPosition` 函数
- `src/frontend/feng-shui-8-bit/src/client/pages/GameplayPage.tsx` - 添加初始位置回调处理
...
**问题**：游戏页面图片加载时，进度条固定在 60% 不动，看起来像卡住了一样。
...
3. **视觉增强**：
   - 外层光晕添加呼吸效果（animate-pulse）
   - 标题添加文字发光
   - 进度条容器添加网格背景纹理
   - 整体间距和尺寸调整

### 影响范围
- `src/frontend/feng-shui-8-bit/src/client/game/GameStage.tsx` - 加载 UI 优化
...
**核心优化**：解决进度条不可见、返回按钮难触达的问题。

**功能实现**：

1. **Sticky 吸顶头部**：
   - 顶部区域（返回按钮 + 标题 + 进度路径）固定在视口顶部
   - 毛玻璃背景效果（blur 12px）
   - 滚动列表时进度信息始终可见
...
- `ChapterSection` 组件：移除 `sortedLevels` 置顶逻辑
- `LevelCard` 组件：动态计算 `shaCompleted` 进度值
- 自动滚动：改为滚动到具体关卡卡片元素
...
1. **useLevelCompletion Hook**：
   - 新增 `isLevelUnlocked(levelId)` 方法
   - 动态计算关卡解锁状态

2. **LevelSelectPage**：
   - 使用 `isLevelUnlocked` 替代静态 `level.locked`
   - ChapterSection 接收解锁/完成状态判断函数
   - LevelCard 接收 `isLocked` 和 `isCompleted` 作为 prop
...
### Bug 修复：Level ID 格式不一致导致关卡无法解锁

**问题**：通关后返回列表页面，关卡状态未更新，下一关也没有自动解锁。

**根因**：Level ID 格式不匹配：
- `levels.ts` 存储格式：`level-1`, `level-2`, ..., `level-10`
- `isLevelUnlocked` 检查格式：`level-01`, `level-02`, ..., `level-09`, `level-10`
- 例如：通关 level-1（存储 `level-1`）后，检查 level-2 解锁状态时查找 `level-01`，导致匹配失败

**修复方案**：移除 `padStart(2, '0')` 填充，确保格式一致：
```tsx
// 修复前
...
### 影响范围
- `src/frontend/feng-shui-8-bit/src/client/stores/LevelCompletionContext.tsx` - 修复 ID 格式
- `src/frontend/feng-shui-8-bit/src/client/pages/LevelSelectPage.tsx`
- `src/frontend/feng-shui-8-bit/src/client/pages/GameplayPage.tsx`
...
**根因分析**：`useLevelCompletion` 是独立 Hook，每个组件有独立的 `completedLevels` 状态。

**修复方案**：将通关状态从独立 Hook 提升到 React Context。

- 新增 `LevelCompletionContext.tsx`：管理全局通关状态
- 删除 `hooks/useLevelCompletion.ts`：避免状态分散
- 更新 `game.tsx`：添加 `LevelCompletionProvider` 包装
- 更新导入路径：`useLevelCompletion` 现在从 Context 导出

**状态**：问题仍未解决，需继续排查

### 影响范围
- `src/frontend/feng-shui-8-bit/src/client/stores/LevelCompletionContext.tsx` - 新增
- `src/frontend/feng-shui-8-bit/src/client/hooks/useLevelCompletion.ts` - 删除
- `src/frontend/feng-shui-8-bit/src/client/game.tsx` - 添加 Provider
- `src/frontend/feng-shui-8-bit/src/client/pages/LevelSelectPage.tsx`
- `src/frontend/feng-shui-8-bit/src/client/pages/GameplayPage.tsx`
